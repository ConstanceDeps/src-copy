...
def classify(datasets, name, clf):
    ...
    not_perfect_ones = np.mean(not_perfect_ones)

    return ratios, number_perfect, not_perfect_ones, bad_pred_but_one, bad_pred_but_zero
...
def main(number_gg):
    ...
    for name, clf in zip(names, classifiers):
        rat, perf, notperf, baad, badzero = classify(datasets, name, clf)
        myfile = open('numpred' + str(name) + 'samples'+ str(number_g) + '.txt', 'w')
        myfile.write('Algorithm is ' + str(name) + '\n')
        myfile.write('Number of subset is ' + str(number_g) + '\n')
        myfile.write(str(rat) + '\n' + 'perfect predictions: ' + str(perf) + '\n' + 'not perfect' +str(notperf) +'\n bad prediction but still one identified ' + str(baad))
        myfile.write('\n' + 'bad prediction 0 identified ' + str(badzero))
        myfile.close()
