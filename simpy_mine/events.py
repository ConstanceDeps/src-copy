"""
This module contains the basic event types used in simpy_mine.
...
.. autosummary::
    ...
    ~simpy_mine.events.Event
    ~simpy_mine.events.Timeout
    ~simpy_mine.events.Process
    ~simpy_mine.events.AnyOf
    ~simpy_mine.events.AllOf
...
from simpy_mine._compat import PY2
...
class Event(object):
    ...
    def __init__(self, env):
        self.env = env
        """The :class:`~simpy_mine.core.Environment` the event lives in."""
        self.callbacks = []
    ...
    def __and__(self, other):
        """Return a :class:`~simpy_mine.events.Condition` that will be triggered if
...
class Timeout(Event):
    """A :class:`~simpy_mine.events.Event` that gets triggered after a *delay* has
    passed.
...
class Process(Event):
    ...
    .. note::
       ...
       generators. You can use :meth:~simpy_mine.core.Environment.exit() as
       a workaround.
...
class ConditionValue(object):
    """Result of a :class:`~simpy_mine.events.Condition`. It supports convenient
...
class AllOf(Condition):
    """A :class:`~simpy_mine.events.Condition` event that is triggered if all of
...
class AnyOf(Condition):
    """A :class:`~simpy_mine.events.Condition` event that is triggered if any of
...
class Interrupt(Exception):
    ...
    :func:`~simpy_mine.events.Process.interrupt()`).
