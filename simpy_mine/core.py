...
from simpy_mine.events import (AllOf, AnyOf, Event, Process, Timeout, URGENT,
                          NORMAL)
...
class BaseEnvironment(object):
    ...
    For example, simpy_mine defines a :class:`Environment` for simulations with
    a virtual time and and a :class:`~simpy_mine.rt.RealtimeEnvironment` that
    ...
    def schedule(self, event, priority=NORMAL, delay=0):
        ...
        There are two default priority values, :data:`~simpy_mine.events.URGENT` and
        :data:`~simpy_mine.events.NORMAL`.
    ...
    def run(self, until=None):
        ...
        - If it is an :class:`~simpy_mine.events.Event`, the method will continue
...
class Environment(BaseEnvironment):
    ...
    def peek(self):
        ...
        :data:`~simpy_mine.core.Infinity` if there is no further event."""
        try:
