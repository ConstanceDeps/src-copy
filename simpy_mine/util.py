...
def start_delayed(env, generator, delay):
    ...
    :meth:`~simpy_mine.core.Environment.process()` starts a process at the current
    simulation time. This helper allows you to start a process after a delay of
    *delay* simulation time units::

        >>> from simpy_mine import Environment
        >>> from simpy_mine.util import start_delayed
...
def subscribe_at(event):
    ...
    a :class:`~simpy_mine.events.Process` to get notified when it terminates.
...
def test():
    """Runs simpy_mine's test suite via `py.test <http://pytest.org/latest/>`_."""
    import os.path
    try:
        import pytest
    except ImportError:
